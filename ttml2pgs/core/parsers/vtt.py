...
class VTTParser:
    ...
    def _parse_payload(self, text: str, root: SpanNode, doc: SubtitleDocument):
        ...
        self._auto_ruby(root, doc)

    # ------------------------------------------------------------------ #
    def _auto_ruby(self, root: SpanNode, doc: SubtitleDocument):
        """
        Convert flattened "漢字(かんじ)" patterns (common in Japanese VTT
        exports that lost their <ruby> markup) into ruby span structures.
        Only applies to Japanese documents; the base is the run of kanji
        immediately before the parenthesis (or the segment after the last
        space, which is discarded as a delimiter).
        """
        lang = (doc.language or detect_doc_lang_hint(doc)) or ''
        if not lang.startswith('ja'):
            return
        _auto_ruby_walk(root)


_KANJI_RE = re.compile(r'[㐀-鿿豈-﫿々ヶ]')
_KANA_RE = re.compile(r'^[぀-ヿー]+$')


def detect_doc_lang_hint(doc: SubtitleDocument) -> str:
    """Heuristic: sniff Japanese from cue text (kana presence)."""
    sample = ''.join(c.plain_text() for c in doc.cues[:50])
    if re.search(r'[぀-ゟ]', sample):
        return 'ja'
    return ''


def _auto_ruby_walk(node: SpanNode):
    new_children: List[SpanNode] = []
    for child in node.children:
        if child.kind == 'text':
            new_children.extend(_auto_ruby_split(child))
        else:
            if child.kind == 'span' and (child.inline_style is None or
                                         child.inline_style.ruby is None):
                _auto_ruby_walk(child)
            new_children.append(child)
    node.children = new_children


def _auto_ruby_split(textnode: SpanNode) -> List[SpanNode]:
    text = textnode.text
    out: List[SpanNode] = []
    buf = ''
    i = 0
    while i < len(text):
        ch = text[i]
        if ch == '(' or ch == '（':
            close = text.find(')' if ch == '(' else '）', i)
            if close > i:
                candidate = text[i + 1:close]
                if _KANA_RE.match(candidate):
                    # find base: chars before '(' back to last space/non-base
                    space_idx = max(buf.rfind(' '), buf.rfind('　'))
                    pre, base = (buf[:space_idx], buf[space_idx + 1:]) \
                        if space_idx != -1 else ('', buf)
                    # base must end with kanji to qualify
                    if base and _KANJI_RE.search(base):
                        if pre:
                            out.append(SpanNode.text_node(pre))
                        container = SpanNode(kind='span',
                                             inline_style=Style(ruby='container'))
                        bnode = SpanNode(kind='span',
                                         inline_style=Style(ruby='base'))
                        bnode.children.append(SpanNode.text_node(base))
                        tnode = SpanNode(kind='span',
                                         inline_style=Style(ruby='text'))
                        tnode.children.append(SpanNode.text_node(candidate))
